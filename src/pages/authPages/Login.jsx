...
function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let loggedUser;
      if (form.role === "seller") {
        loggedUser = await sellerLogin({ email: form.email, password: form.password });
        toast.success(`Welcome back, ${loggedUser.fullName || loggedUser.name || "Seller"}!`);
        navigate("/seller/dashboard");
      } else {
        loggedUser = await buyerLogin({ email: form.email, password: form.password });
        toast.success(`Welcome back, ${loggedUser.fullName || loggedUser.name || ""}!`);
        navigate("/");
      }
    } catch (error) {
  ...
  return (
    <AuthLayout
      side={
        <div className="h-full bg-[#F8E7EC] p-8 lg:p-12 flex flex-col justify-between">
          <div>
            <p className="text-[#D90452] font-black text-xl">GiftBoxy</p>
            ...
            <p className="mt-6 text-[#7A7272] max-w-sm leading-relaxed text-sm lg:text-base">
              Save gifts, message sellers and continue your personalized shopping journey.
            </p>
          </div>
          <img
    ...
    >
      <div className="flex items-center justify-between">
        <p className="text-[#D90452] font-black text-lg">GiftBoxy</p>
      ...
      <div className="mt-10 lg:mt-16">
        <p className="text-[#D90452] text-xs font-black uppercase tracking-[0.3em]">Sign In</p>
      ...
      <div className="mt-8 bg-[#F8F1EC] rounded-full p-1 grid grid-cols-2">
        <button
          type="button"
          onClick={() => { setForm({ ...form, role: "buyer" }); setErrors({ email: "", password: "", general: "" }); }}
          className={`py-3 rounded-full font-black transition ${form.role === "buyer" ? "bg-[#D90452] text-white" : "text-[#6F6262]"}`}
        >
          Buyer
        </button>
        <button
          type="button"
          onClick={() => { setForm({ ...form, role: "seller" }); setErrors({ email: "", password: "", general: "" }); }}
          className={`py-3 rounded-full font-black transition ${form.role === "seller" ? "bg-[#D90452] text-white" : "text-[#6F6262]"}`}
        >
          Seller
        </button>
      </div>
      ...
      <form onSubmit={handleSubmit} className="mt-8 space-y-5">
        ...
        <button
          disabled={loading}
          className="w-full bg-[#D90452] text-white py-4 rounded-full font-black shadow-lg shadow-pink-200 hover:scale-[1.01] transition disabled:opacity-60"
        >
